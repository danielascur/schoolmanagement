...
collapse (mean) zmgmt_wms_practices zmgmt_broad_practices zmgmt_common_practices zmgmt_wms_and zmgmt_wms_pca math reading science [w=stu_fac], by(uschool_id cnt)


* Plotting correlation of scores and management for WMS and Common indices
local indices common_practices wms_and wms_pca
	...
	preserve
		...
		* Looping through countries and building quartiles variables
		...
		foreach c of local lcty {
			...
			cap drop gmi_`c'
			cap xtile qgmi_`c' = zmgmt_`i' if cnt == "`c'", nq(4)
		...
		* For common, increading the range of y axis from 5 to 6
		if "`i'" == "common_practices" { 
			...
			if "`i'" == "wms_and" {
				graph export "analysis/results/figure_a1c.pdf", as(pdf) replace
			}
			else if "`i'" == "wms_pca" {
				graph export "analysis/results/figure_a1d.pdf", as(pdf) replace
			}	
		}
	restore
}	
